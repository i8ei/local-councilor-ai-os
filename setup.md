...
このプロファイルは、Vault、AIエージェント、保存規約が [`claude-obsidian-setup`](https://github.com/i8ei/claude-obsidian-setup) で構築済みであることを前提とする。基盤を置き換えず、地方議会実務の棚、プロファイル、ワークフロー、公開情報の検索層を上に追加する。
...
対象自治体、都道府県ヒント、新規の出力先、外部通信を確認する。オンライン実行には`ESTAT_APPID`が必要である。[e-Stat APIの利用ガイド](https://www.e-stat.go.jp/api/api-info/api-guide)から無料の利用登録を行い、マイページでアプリケーションIDを発行する。IDは環境変数として実行時だけ渡し、ノート、実行報告、キャッシュ、データベースへ書かない。
...
自治体コードの一意性、DBの整合性、指標の値、時点、定義、出典、`authority_map.yaml`に値が複製されていないことを確認する。同じキャッシュからオフライン再構築し、行数と主要値が一致することも確認する。不一致は隔離し、最新値として使わない。
