...
自治体固有層の入口がまだ分からない場合は、同梱registryの公式ホームURLを起点に
少量HTMLだけを調べるpreflightを先に実行する。

```bash
python3 -m bootstrap.cli.preflight \
  --prefecture '佐賀県' \
  --output /tmp/saga-preflight.json \
  --cache-dir /tmp/saga-preflight-cache
```
...
- `--cross-check` で同年度の決算カード年度ページと対象都道府県XLSXを発見し、XLSXの署名・Content-Typeを検証して、URL、SHA-256、取得時刻、キャッシュ位置を記録
