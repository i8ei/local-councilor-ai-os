...
from modules.budget_review import (
    csv_templates,
    ingest_csv,
    insights,
    verify_totals,
...
class BudgetPipelineTests(unittest.TestCase):
    ...
    def test_child_sum_rejects_non_allowlisted_field(self) -> None:
        with closing(sqlite3.connect(":memory:")) as connection:
            with self.assertRaisesRegex(ValueError, "Unsupported amount field"):
                verify_totals._child_sum(
                    connection,
                    None,  # type: ignore[arg-type]
                    "kan",
                    "amount; DROP TABLE budget_line",
                )
