...
この契約は、PDF、表抽出、OCR、AI、人手入力など任意の方法で読み取った決算数値を、検算可能な SQLite へ入れるための受け皿である。PDF から数字を読む方法は自治体、年度、会計、資料公開範囲によって異なるため、本モジュールは抽出器を汎用化しない。共通化するのは、抽出後に満たすべき列、粒度、原典位置、検証状態である。

## ファイル種別

`ingest_csv.py` は次の3種類を受け付ける。
...
CSVを投入しただけでは検証済みではない。少なくとも次を満たすまで、対外利用可能な決算DBと呼ばない。
