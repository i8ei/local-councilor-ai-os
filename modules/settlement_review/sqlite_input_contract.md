...
**翌年度繰越の扱い（2026-08-25 実戦検証で確定）**: 多くの帳票は節行に翌年度繰越列を持たない（太良町R7を含む）。その場合 `section_carryover_amount` は全節 0 と記録し、繰越は目行の値（予算現額-支出済-不用の残差）のみを `item_carryover_amount` に持たせる。`verify_totals` は「節合計0 ≠ 目繰越>0」の組み合わせを許容する（節への分解が不可能な帳票構造のため）。逆に節側にだけ繰越がある（目側0）場合は記帳不整合として不合格。
