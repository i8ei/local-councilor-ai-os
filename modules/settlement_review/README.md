...
本モジュールが提供するのは、抽出後の数値をSQLiteへ格納するための入力契約、SQLiteスキーマ、差額ゼロ検算、分析候補生成、公開・非公開境界、ありがちな失敗パターンである。CSVの扱いは[入力契約](sqlite_input_contract.md)のとおりである。
...
対外利用できるのは、公式公開資料に基づき差額ゼロ検算と人の原典確認を通った値だけである。境界の詳細は[公開・非公開境界](public_private_boundary.md)に従う。
