...
全国 1,741 自治体の議事録・例規集・当初予算・決算の公式入口を構造化したプロファイル（`source_profiles/`）を内蔵。`python3 -m bootstrap.cli <自治体名>` を叩くだけで、**全国すべての自治体で国勢調査・総務省の公式指標を取得**し、議事録・例規の取込コマンドを即座に提示します（※議事録・例規は全体の過半数にあたる 2,184 件が即取込可能な `ready` 状態。未検証の自治体もプロファイルの追記で対応可能）。
