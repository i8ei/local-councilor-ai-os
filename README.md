...
現在は **v0.1** です。運用設計の中核である `way-of-working/` 10章、実務手順の `workflows/` 7本、安全原則、証拠の扱い、ブートストラップの検証済み設計、データ契約例、実務テンプレートを公開済みです。実装モジュールは順次追加します。

特に次の実装は、現時点では完成品ではありません。

- 自治体名からのブートストラップCLI
- 議事録・例規のベンダー別アダプター
- 予算・決算PDFの照合付き取込パイプライン
- SQLiteからのコンテキストパック生成
