...
これは、地方議員がAIを便利な文章生成機として使うためのテンプレート集ではありません。住民の声、公開資料、議事録、予算決算、例規、統計、質問、答弁、その後の実装を、AIとObsidianとSQLiteでつなぎ直すための運用OSです。AIに判断を委ねるのではなく、判断に至る根拠、迷い、検算、更新、失敗を残す。公開情報と個人情報を混ぜず、原典とデータベースとノートの役割を分ける。質問して終わりではなく、答弁後の予算化と実施まで追う。このリポジトリは、そういう働き方を実データ抜きで蒸留したものです。

## これは何か

`local-councilor-ai-os` は、日本の基礎自治体で活動する地方議員の実務を題材にした、意見のある参照実装です。公開資料を収集し、再構築可能なSQLite／FTS5検索層をつくり、判断の正本をObsidianに残すところまでを一つの運用として扱います。
...
- 例規ベンダー固有アダプター、ベンチマークDBの比較条件プリセット
- 議事録ベンダーアダプターの対応拡大
