...
これは、地方議員がAIを便利な文章生成機として使うためのテンプレート集ではありません。地方議員の実務の中で実践的に鍛えた、AI・Obsidian・SQLiteによる調査、検算、判断記録、答弁後追跡の運用を、実データを除いて蒸留したものです。AIに判断を委ねるのではなく、判断に至る根拠、迷い、検算、更新、失敗を残す。公開情報と個人情報を混ぜず、原典とデータベースとノートの役割を分ける。質問して終わりではなく、答弁後の予算化と実施まで追います。
...
したがって、たとえば決算PDFから数字を抜き取る処理は、自治体や資料ごとのAI・人手・OCRに委ねます。このリポジトリが提供するのは、その結果を入れるCSV契約、DB構造、原典位置、差額ゼロ検算、公開・非公開境界です。
...
- 決算審査モジュールの標準スキーマ、CSV契約、標準CSV取込、差額ゼロ検算、分析候補生成
...
- 決算PDF抽出そのものではなく、正規化CSV契約、公開・非公開境界、失敗パターン、分析候補生成の実戦検証と閾値設計を深める
- ベンチマークDBの比較条件プリセット。同一年度、同一定義で安全に比較できる指標の組み合わせを定義済みにする
- 例規のベンダーアダプター。静的汎用型に加えて、採用の多い例規サイトの構造を知識として持つアダプターを追加する
- 議事録の静的設定プリセット集。よくある公開レイアウト向けの設定例を同梱する

議事録のベンダーアダプター追加は、対象システムを実際に使う自治体の利用者が現れ、実戦検証が可能になった時点で対応します。検出だけ可能で取込未対応の場合、その状態を正直に返すのが現在の仕様です。
...
| `modules/` | 議事録、決算審査、例規、比較分析の各モジュール |
...
ルートの `python3 -m unittest discover` は `bootstrap/` のテストだけを検出します。`modules/` はパッケージではなく、サブディレクトリ名にハイフンを含むため、各モジュールのテストは個別に起動する必要があります。`run_tests.sh` は bootstrap と各モジュールのテスト、決算の差額ゼロ検算までをまとめて実行し、いずれかが失敗すると終了コード1を返します。
