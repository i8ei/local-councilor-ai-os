...
![地方議員AI運用OSの概要 — 議会議事録や条例、予算・決算、公開データを見立て・ツボ・手当てにつなぐ](docs/assets/local-councilor-ai-os-og-1200x630.png)
...
いわば、地方議員のための「知的重機」です。大量の資料を掘り、数字と議論を運び、過去と現在をつなぎますが、どこを掘り、何に使うかを決める操縦者は議員本人です。
...
地方議員の実務の中で使いながら整えてきた、仕事の進め方と道具をまとめています。
統計値、取得原本、住民情報、利用者のDBは含めません。例外として、初回探索を短くする
ため、全国1,741自治体の公開URLと構造だけを縮約した
[観測snapshot](bootstrap/observatory/README.md)を同梱しています。
...
このOSでは、地域の困りごとを制度・事業・運用の構造として整理し、次の順で働きかける先を探します。

1. **見立て** — 何が、どこで詰まっているかを、事実と意見を分けて整理する
2. **ツボ** — 町・県・国の制度をたどり、どこへ働きかければ一番効くかを見つける
3. **手当て** — 期待する効果だけでなく、副作用や反対論まで含めた変更案を書く

責任の所在を曖昧にするものではありません。法的・政治的・実務的な責任がある場合は、事実と根拠に基づいて検証します。その検証と、どこを変えれば状況が動くかという提案設計を分けて扱います。
...
初見の利用者は、まず読み取り専用診断を行い、次の順に進めると迷いにくい。
...
# 4. 議事録・例規・予算・決算の現在の入口を少量HTMLで確認
...
- 実在する自治体の統計値、取得原本、住民情報、議員個人の運用環境を配布するリポジトリではありません。公開URLの観測snapshotは探索候補であり、現在の公式入口や内容を保証しません。
...
議事録、例規、予算、決算の入口は、全国1,741自治体を同じ条件で浅く観測した
snapshotから候補を取り出せます。候補は探索順を決めるだけで、過去に見えたURLだけを
`ready`にはしません。preflightが現在の公式ホームを取得し、同一公式hostから辿れる
ことを再確認します。詳しくは
[自治体ブートストラップ](bootstrap/README.md)と
[全国自治体観測ヒント](bootstrap/observatory/README.md)を参照してください。
...
| 2 | 議事録・例規 | 全国観測の候補をlive preflightで再確認し、CMS／例規サイト別アダプターへ接続する | snapshotは現在性を保証しない。全国一律の公開仕様ではなく、サイトごとの実装と保守が必要 |
...
現在は **v0.2系** です。最新リリースは v0.2.0 です。公開済み・実装済みの範囲は次のとおりです。
...
- 全国1,741自治体の公開構造を約1.34MBへ縮約した観測snapshot。過去観測だけで`ready`にしないlive preflightの優先順位として利用
...
v0.2.0では、OS制御層、全国自治体registry、自治体固有入口のpreflight、全国観測snapshot、ツボ探しまでを一つの利用可能な基準線として固定しました。次の実戦検証は、v0.2系の改善として継続します。
...
| [`bootstrap/`](bootstrap/README.md) | 自治体名から全国共通データ基盤を立ち上げ、固有資料の入口を診断する設計 |
| [`bootstrap/observatory/`](bootstrap/observatory/README.md) | 全国1,741自治体の公開URL・構造を縮約した観測snapshot、信頼境界、更新手順 |
