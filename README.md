...
`--output` に指定したJSONには、自治体ごとに4種の入口の判定が入ります。`ready` は現在の公式ページから入口を確認できた状態、`needs_attention` は理由付きで止めた状態です。値は推測せず、必ず `reason` と `evidence` を残します。以下は形式を示すために省略・抽象化した例で、特定自治体の実行結果ではありません。
