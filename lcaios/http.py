...
MINUTES_USER_AGENT = "local-councilor-ai-os minutes ingester (research; low rate)"
...
class CacheTier(enum.Enum):
    """Freshness policy for a cached response."""
...
class HttpClient:
    ...
    def retrieval_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            and all(
                item["status"] in {"fetched", "refreshed"} for item in self.fetch_log
            ),
    ...
    def _cache_is_usable(self, result: FetchResult, tier: CacheTier) -> bool:
        if self.offline or tier is CacheTier.DOCUMENT:
            return True
        fetched_at = _parse_fetched_at(result.fetched_at)
        if fetched_at is None:
            return False
        age = (datetime.now(timezone.utc) - fetched_at).total_seconds()
        return age <= self.index_ttl_seconds

    def _read_usable_cached(
        self,
        cache_key: str,
        tier: CacheTier,
    ) -> tuple[FetchResult, int] | None:
        cached = self._read_cached(cache_key, accepted_statuses=range(200, 300))
        if cached is None or not self._cache_is_usable(cached[0], tier):
            return None
        return cached
    ...
    def _read_response_body(self, response: Any, url: str) -> bytes:
        ...
        if raw_length is not None:
            ...
            if (
                ...
                raise FetchError(
                    f"応答サイズが上限 {self.max_response_bytes} バイトを"
                    f"超えています: {url}"
                )
